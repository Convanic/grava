...
- `GET /api/v1/game/me/history?days` — zeitlicher Verlauf der eigenen Revier-Kennzahlen für den iOS-Chart (**Bearer**, sonst `401`; `backend/GAME_HISTORY_BACKEND.md`). `days` (Default 365, geklammert 1…730) = Fenster rückwärts ab heute (UTC). Effektiver Claimant wie `/game/me`. Antwort `{ "points": [ { "date": "YYYY-MM-DD", "held_edges", "pioneered_edges", "held_length_m" } ] }` — ein Punkt pro Tag, chronologisch; `date` ist ein **reines Kalenderdatum** (kein Zeitstempel). Punkte liefert der tägliche Snapshot (Cron `game:snapshot-daily`, Tabelle `game_user_stats_daily`); der erste Lauf backfillt die Vergangenheit aus `game_edge.owner_since`/`discovered_at` (Pionier exakt, „held" = Wachstumskurve des heute gehaltenen Reviers). Leere Antwort, bis Snapshots vorliegen.
